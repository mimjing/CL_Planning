...
class VBD(pl.LightningModule):
    ...
    def __init__(
        ...
        self._agents_len = config['agents_len']
        self._action_len = config['action_len']
        self._diffusion_steps = config['diffusion_steps']
        self._encoder_layers = config['encoder_layers']
        self._encoder_version = config.get('encoder_version', 'v1')
        self._action_mean = config['action_mean']
        self._action_std = config['action_std']
        
        self._train_encoder = config.get('train_encoder', True)
        self._train_denoiser = config.get('train_denoiser', True)
        self._train_predictor = config.get('train_predictor', True)
        self._with_predictor = config.get('with_predictor', True)
        self._prediction_type = config.get('prediction_type', 'sample')
        self._schedule_type = config.get('schedule_type', 'cosine')
        self._replay_buffer = config.get('replay_buffer', False)
        self._embeding_dim = config.get('embeding_dim', 5) # By default, the embed is the noised trajectory so the dimension is 5
        ...
        self.noise_scheduler = DDPM_Sampler(
            ...
            s = config.get('schedule_s', 0.0),
            e = config.get('schedule_e', 1.0),
            tau = config.get('schedule_tau', 1.0),
            scale = config.get('schedule_scale', 1.0),
        )
    ...
    def calculate_metrics_denoise(self, 
            ...
            """
            Calculates the denoising metrics_before for the predicted trajectories.
    ...
    def calculate_metrics_predict(self,
            ...
            """
            Calculates the metrics_before for predicting goal trajectories.
