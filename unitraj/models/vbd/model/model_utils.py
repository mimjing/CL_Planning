...
def batch_transform_polylines_to_local_frame(polylines):
    ...
    """
    B, M, N, _ = polylines.shape
    assert N >= 1, "Each polyline must have at least one point"
    x = polylines[..., 0]
    y = polylines[..., 1]
    theta = polylines[..., 2]
    # print("Theta shape:", theta.shape, "Theta min/max:", theta.min(), theta.max())
    # 检查是否存在NaN/Inf
    assert not torch.isnan(theta).any(), "Theta contains NaN!"
    assert not torch.isinf(theta).any(), "Theta contains Inf!"
...
def inverse_kinematics(
    ...
    action_valid = agents_future_valid[..., :1] & agents_future_valid[..., 1:] # 第一个时间步和当前时间步同时为真则当前时间步为真
    ...
    action_valid = action_valid.any(dim=-1) # 有一个时间步为真则为真
...
def roll_out(
        current_states: torch.Tensor, #[1,16,8]
        actions: torch.Tensor, #[1,16,40,2]
        dt: float = 0.1,
        action_len: int = 5,
