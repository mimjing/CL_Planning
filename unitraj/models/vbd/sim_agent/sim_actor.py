...
from unitraj.models.vbd.sim_agent.reward_balance import SafetyBalancedReward
from unitraj.models.vbd.sim_agent.utils import *
from unitraj.models.vbd.model.model_utils import roll_out
from unitraj.models.vbd.sim_agent.guidance_metrics import *

"""通过继承 VBD，增加了对轨迹生成过程的控制和引导，使得轨迹生成更加灵活和可定制"""

class VBDTest(VBD):
    def __init__(self, 
        cfg: dict,
        early_stop: int = 0, # 提前停止参数
        skip: int = 1, # 跳过扩散步骤
        reward_func: nn.Module = None,
        # reward_func: nn.Module = torch.nn.ModuleList([TrackingReward(),]),
        guidance_iter: int = 5,
        guidance_end: int = 1,
    ...
    def guidance(self, x_t: torch.Tensor, c: dict, t: int, **kwargs) -> Tuple[Dict[str, torch.Tensor], torch.Tensor, Dict[str, np.ndarray]]:
        ...
        reward_system = SafetyBalancedReward()
        for _ in range(self.guidance_iter):
            with torch.enable_grad():
                mu.requires_grad_()
                ...
                reward_guide.append(total_rewards.detach().cpu().numpy())
                grad_guide.append(grad.detach().cpu().numpy())
        ...
        for iter_round in range(self.guidance_iter):
            for iter in range(ego_iter+adv_iter):
                with torch.enable_grad():
                    ...
                    # min_values = torch.tensor([-1.0, -0.15], device=mu.device)  # 自动匹配设备
                    # max_values = torch.tensor([1.0, 0.15], device=mu.device)
                    # mu = torch.clamp(mu, min=min_values, max=max_values)
    ...
    @torch.no_grad()
    def sample_denoiser(self, batch, num_samples=1, x_t = None, use_tqdm = False, fix_t: int = -1, calc_loss: bool = False, **kwargs):
        """
        ...
        for t in diffusion_steps:
            ...
            if t <= self.guidance_start and t >= self.guidance_end and \
                ...
                denoiser_outputs, x_t = self.step_denoiser(
                    x_t = x_t,  # [1，16，80，2]动作
                    c = encoder_outputs, 
                    t = fix_t if fix_t >= 0 else t,
                )
                guide = None

            # Calculate the loss and metrics_before
            if calc_loss: 
