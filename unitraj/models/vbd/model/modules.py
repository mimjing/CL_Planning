...
class GoalPredictor(nn.Module):
    ...
    def forward(self, inputs):
        anchors_points = inputs['anchors'][:, :self._agents_len] # inputs是编码器输出
        anchors = self.anchor_encoder(anchors_points)   # anchor在encoder没进行处理
        encodings = inputs['encodings']  # 众encoding的再一层encoder
        ...
        for i in range(self._agents_len):
            ...
            query_content = query_content + query[:, i]  # 残差连接，用于保留原始查询向量的特征，避免信息在多层注意力更新中丢失
        ...
        actions = torch.stack(actions, dim=1)  # 将每个智能体的动作和得分堆叠起来，得到最终的输出
...
class Denoiser(nn.Module):
    ...
    def forward(self, encoder_inputs, noisy_actions, diffusion_step, rollout = True):
        ...
        if type(diffusion_step) == int:
            ...
            ) # 生成一个形状为(1,16)的张量，其中每个元素都是diffusion_step 49
        else:
        ...
        mask = torch.cat([agents_mask, maps_mask, traffic_lights_mask], dim=-1)  # 拼接掩码信息

        # denoise step
        noise_level = self.noise_level_embedding(diffusion_step)  # 一个形状为(1, 16, 256)的张量，其中每个噪声水平的索引都被转换为了一个256维的嵌入向量
        if rollout: # embedding[1,16,80,5]
...
class MapEncoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        assert inputs.shape[2] > 0, "Polyline must have at least one point"
        output = self.point(inputs[..., :3])   # 点位置数据
